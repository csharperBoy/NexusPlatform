...
export function SmartDataGridCore<T>({ toolbarSlot, paginationSlot, emptyMessage = 'داده‌ای وجود ندارد', className = '' }: CoreProps) {
  ...
  const addedKeys = instance.pluginState.editing?.addedKeys || new Set();
  const rowErrors = instance.pluginState.editing?.rowErrors || new Map();
