        ...
        <div className="p-5 flex items-center justify-between bg-gray-50/50">
          ...
          <div className="flex gap-2">
            ...
            {step === "loading_preview" && (
              <button
                disabled
                className="flex items-center gap-2 px-5 py-2.5 bg-blue-400 text-white rounded-lg font-medium"
              >
            ...
            {step === "applying" && (
              <button
                disabled
                className="flex items-center gap-2 px-5 py-2.5 bg-emerald-400 text-white rounded-lg font-medium"
              >
              ...
              <div className="flex gap-4 text-sm font-semibold">
                <button
                  onClick={() => {
                    const all = [
                      ...bundle.addCommands,
                      ...bundle.updateCommands,
                      ...bundle.deleteCommands,
                    ].map((i) => i.id);
            ...
            {applyResult.data && (
              <div className="flex gap-6 text-sm mb-4 bg-white p-4 rounded-lg border shadow-sm">
                <span className="text-emerald-600 font-bold">
                  ➕ افزوده: {applyResult.data.addedCount}
                </span>
                <span className="text-blue-600 font-bold">
                  ✏️ بروزرسانی: {applyResult.data.updatedCount}
                </span>
                <span className="text-rose-600 font-bold">
                  ➖ حذف: {applyResult.data.deletedCount}
                </span>
              </div>
            )}
            ...
            {(applyResult.successMessages?.length ?? 0) > 0 && (
              <div className="mb-3 text-emerald-700 text-sm bg-emerald-50/50 p-3 rounded-lg border border-emerald-100">
                ...
                <ul className="list-disc list-inside space-y-1">
                  {applyResult.successMessages?.map((msg, idx) => (
                    <li key={idx}>{msg}</li>
                  ))}
                </ul>
              </div>
...
EntitySyncCard.displayName = "EntitySyncCard";
...
export const SyncPage: React.FC = () => {
  ...
  const employmentRef = useRef<EntitySyncCardHandle>(null);
    ...
    <div className="max-w-5xl mx-auto p-4 sm:p-6 lg:p-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-2 text-sm leading-relaxed max-w-3xl">
          ...
          ابتدا پیش‌نمایش تغییرات را دریافت کنید، موارد دلخواه را تایید کرده و سپس آن‌ها را
          در پایگاه داده اعمال نمایید.
        </p>

        {/* دکمه همگام‌سازی کلی */}
        <div className="mt-6 p-4 bg-gradient-to-l from-indigo-50 to-blue-50 border border-indigo-100 rounded-xl">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <button
              ...
              className={`shrink-0 flex items-center gap-2 px-6 py-3 rounded-lg font-bold shadow-md transition active:scale-95 ${
                mainSyncRunning
