    ...
    public class ModuleInitializer : IHostedService
    {
        ...
        public async Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                await SchedulerSeedData.SeedSchedulerForAuthorizationAsync(resourceService, permissionService, roleService, _logger);
                await SchedulerSeedData.SeedSchedulersForNavigationAsync(menuService, _logger);

                _logger.LogInformation("Scheduler module initialization completed successfully.");
            }
            catch (Exception ex)
            {
                // 📌 ثبت خطا در صورت شکست عملیات
                _logger.LogError(ex, "An error occurred while initializing the Scheduler module");
                throw;
            }
        }
