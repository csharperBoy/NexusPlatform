            ...
            CancellationToken cancellationToken)
        {
            try
            {
                ...
                IReadOnlyList<PostContactDto> result = posts.Select(post => new PostContactDto
                {
                    ...
                    OfficePhone = contactList.Where(a => a.ProfileId == post.ProfileId && a.ContactType == ContactTypeEnum.OfficePhone && a.IsCurrent)?.Select(a => a.Value).ToList(),
                    OrgMobile = contactList.Where(a => a.ProfileId == post.ProfileId && a.ContactType == ContactTypeEnum.OrganizationMobile && a.IsCurrent)?.Select(a => a.Value).ToList(),
                    OrgEmail = contactList.Where(a => a.ProfileId == post.ProfileId && a.ContactType == ContactTypeEnum.Email && a.IsCurrent)?.Select(a => a.Value).ToList(),
