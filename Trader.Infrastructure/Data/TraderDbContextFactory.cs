...
namespace Trader.Infrastructure.Data
{
    /*
     ...
     ✅ نکات کلیدی:
     - از IDesignTimeDbContextFactory<TraderDbContext> ارث‌بری می‌کند.
     ...
     2. این Factory فراخوانی می‌شود تا یک نمونه از TraderDbContext ساخته شود.
    ...
    public class TraderDbContextFactory : IDesignTimeDbContextFactory<TraderDbContext>
    {
        public TraderDbContext CreateDbContext(string[] args)
        {
            ...
            optionsBuilder.UseSqlServer(conn, b =>
            {
                ...
                // 📌 تعیین جدول تاریخچه Migrationها در اسکیمای "Trader"
