...
FROM            hr.Location INNER JOIN
                         ...
                         contact.PostContacts AS PostContact_Phone ON hr.Post.Id = PostContact_Phone.FkPostId AND PostContact_Phone.ContactType = 0 AND PostContact_Phone.IsCurrent = 1 LEFT OUTER JOIN
                         contact.PostContacts AS PostContact_Mobile ON hr.Post.Id = PostContact_Mobile.FkPostId AND PostContact_Mobile.ContactType = 1 AND PostContact_Mobile.IsCurrent = 1 LEFT OUTER JOIN
                         contact.PostContacts AS PostContact_Fax ON hr.Post.Id = PostContact_Fax.FkPostId AND PostContact_Fax.ContactType = 2 AND PostContact_Fax.IsCurrent = 1 LEFT OUTER JOIN
                         contact.PostContacts AS PostContact_Email ON hr.Post.Id = PostContact_Email.FkPostId AND PostContact_Email.ContactType = 3 AND PostContact_Email.IsCurrent = 1 LEFT OUTER JOIN
                         ...
                         contact.PartyContacts AS PartyContacts_Mobile ON people.Parties.Id = PartyContacts_Mobile.FkPartyId AND PartyContacts_Mobile.ContactType = 1 AND PartyContacts_Mobile.IsCurrent = 1 LEFT OUTER JOIN
                         contact.PartyContacts AS PartyContacts_Email ON people.Parties.Id = PartyContacts_Email.FkPartyId AND PartyContacts_Email.ContactType = 2 AND PartyContacts_Email.IsCurrent = 1 LEFT OUTER JOIN
                         contact.PartyContacts AS PartyContacts_Phone ON people.Parties.Id = PartyContacts_Phone.FkPartyId AND PartyContacts_Phone.ContactType = 0 AND PartyContacts_Phone.IsCurrent = 1 LEFT OUTER JOIN
                         contact.PartyContacts AS PartyContacts_Address ON people.Parties.Id = PartyContacts_Address.FkPartyId AND PartyContacts_Address.ContactType = 3 AND PartyContacts_Address.IsCurrent = 1 LEFT OUTER JOIN
                         contact.EmploymentContacts AS EmploymentContact_Phone ON hr.[Employment].Id = EmploymentContact_Phone.FkEmploymentId AND EmploymentContact_Phone.ContactType = 0 AND 
                         EmploymentContact_Phone.IsCurrent = 1 LEFT OUTER JOIN
                         contact.EmploymentContacts AS EmploymentContact_Mobile ON hr.[Employment].Id = EmploymentContact_Mobile.FkEmploymentId AND EmploymentContact_Mobile.ContactType = 1 AND 
                         EmploymentContact_Mobile.IsCurrent = 1 LEFT OUTER JOIN
                         contact.EmploymentContacts AS EmploymentContact_Fax ON hr.[Employment].Id = EmploymentContact_Fax.FkEmploymentId AND EmploymentContact_Fax.ContactType = 2 AND 
                         EmploymentContact_Fax.IsCurrent = 1 LEFT OUTER JOIN
                         contact.EmploymentContacts AS EmploymentContact_Email ON hr.[Employment].Id = EmploymentContact_Email.FkEmploymentId AND EmploymentContact_Email.ContactType = 3 AND EmploymentContact_Email.IsCurrent = 1