CREATE VIEW [dbo].[Employement_Info_View]
AS
SELECT        people.naturalPersons.NationalCode, people.naturalPersons.FirstName, people.naturalPersons.LastName, hr.[ Employment].EmployeeCode, hr.[ Employment].EffectiveFrom AS Employee_EffectiveFrom, 
                         ...
                         PostContact_Mobile.Value AS PostContact_Mobile, PostContact_Email.Value AS PostContact_Email, PostContact_Fax.Value AS PostContact_Fax, 
...
FROM            hr.Location INNER JOIN
                         ...
                         people.PartyContacts AS PartyContacts_Mobile ON people.Parties.Id = PartyContacts_Mobile.FkPartyId AND PartyContacts_Mobile.ContactType = 1 LEFT OUTER JOIN
                         people.PartyContacts AS PartyContacts_Email ON people.Parties.Id = PartyContacts_Email.FkPartyId AND PartyContacts_Email.ContactType = 2 LEFT OUTER JOIN
                         people.PartyContacts AS PartyContacts_Phone ON people.Parties.Id = PartyContacts_Phone.FkPartyId AND PartyContacts_Phone.ContactType = 0 LEFT OUTER JOIN
                         people.PartyContacts AS PartyContacts_Address ON people.Parties.Id = PartyContacts_Address.FkPartyId AND PartyContacts_Address.ContactType = 3
