...
{
    public class GetUserRolesQueryHandler : IRequestHandler<GetUserRolesQuery, Result<IList<RoleDto>>>
    {
        private readonly IRoleInternalService _roleService;
        public GetUserRolesQueryHandler(IRoleInternalService roleService)
            => _roleService = roleService;

        public async Task<Result<IList<RoleDto>>> Handle(GetUserRolesQuery request, CancellationToken ct)
        {
            var roles = await _roleService.GetUserRolesAsync(request.UserId);
            return Result<IList<RoleDto>>.Ok(roles);
        }
    }
}
