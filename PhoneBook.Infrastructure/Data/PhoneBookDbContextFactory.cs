...
namespace PhoneBook.Infrastructure.Data
{
    /*
     ...
     ✅ نکات کلیدی:
     - از IDesignTimeDbContextFactory<HRDbContext> ارث‌بری می‌کند.
     ...
     2. این Factory فراخوانی می‌شود تا یک نمونه از HRDbContext ساخته شود.
    ...
    public class PhoneBookDbContextFactory : IDesignTimeDbContextFactory<PhoneBookDbContext>
    {
        public PhoneBookDbContext CreateDbContext(string[] args)
        {
            ...
            optionsBuilder.UseSqlServer(conn, b =>
            {
                ...
                b.MigrationsAssembly(typeof(PhoneBookDbContext).Assembly.GetName().Name);
